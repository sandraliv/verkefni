...
public class RecipeController {
   ...
   /* @PostMapping("/{id}/aaa")
    public ResponseEntity<String> addR(@PathVariable int recipeId, HttpSession session, int score){
        Optional<Recipe> or = recipeService.findById(recipeId);
        Recipe recipe;
        Rating rating;
        if(or.isPresent()){
            recipe = or.get();
        } else {
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body("Problem adding rating");
        }
        User user = (User)session.getAttribute("user");
        if(user != null){
            user.setRatings(addRating(recipeId, score));
            userService.save(user);
            return ResponseEntity.status(HttpStatus.ACCEPTED).body("Favourite added to logged in user");
        }
        return ResponseEntity.status(HttpStatus.NOT_FOUND).body("Could not update user");
    ...
    // Find recipes by category and subcategory
   /* @GetMapping("/findRecipesBySubAndCategory")
    public ResponseEntity<List<Recipe>> getRecipesByCategoryAndSubcategory(
            @RequestParam(value = "categoryId", required = false) Long categoryId,
            @RequestParam(value = "categoryName", required = false) String categoryName,
            @RequestParam(value = "subcategoryId", required = false) Long subcategoryId,
            @RequestParam(value = "subcategoryName", required = false) String subcategoryName) {

        // Initialize category and subcategory to null
        Category category = null;
        Subcategory subcategory = null;

        // Handle Category: If categoryId is provided, use it. If categoryName is provided, use it.
        if (categoryId != null) {
            ...
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(null); // Neither categoryId nor categoryName provided
        }

        // Handle Subcategory: If subcategoryId is provided, use it. If subcategoryName is provided, use it.
        if (subcategoryId != null) {
            Optional<Subcategory> subcategoryOpt = subcategoryService.findBySubcategoryId(subcategoryId);
            if (!subcategoryOpt.isPresent()) {
                return ResponseEntity.status(HttpStatus.NOT_FOUND).body(null); // Subcategory not found by ID
            }
            ...
            if (subcategory == null) {
                return ResponseEntity.status(HttpStatus.NOT_FOUND).body(null); // Subcategory not found by name
            }
        } else {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(null); // Neither subcategoryId nor subcategoryName provided
        }

        // Fetch recipes by category and subcategory
        List<Recipe> recipes = recipeService.findByCategoryAndSubcategory(category.getId(), subcategory.getId());
        return ResponseEntity.ok(recipes);
    }*/
    ...
    public ResponseEntity<List<Recipe>> getRecipesByCategory(@RequestParam("categoryId") Long categoryId) {
        List<Recipe> recipes = recipeService.findByCategoriesId(categoryId);
        return ResponseEntity.ok(recipes);
    }*/
